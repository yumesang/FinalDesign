package com.finalDesign.flowtaskinfo.service.impl;
...
@Service("flowTaskInfoService")
@Transactional
public class FlowTaskInfoServiceImpl extends CommonServiceImpl implements FlowTaskInfoServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(FlowTaskInfoEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(FlowTaskInfoEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(FlowTaskInfoEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(FlowTaskInfoEntity t){
		...
		map.put("pass_date", t.getPassDate());
		map.put("ceate_user_id", t.getCeateUserId());
		return map;
	}
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,FlowTaskInfoEntity t){
 		...
 		sql  = sql.replace("#{pass_date}",String.valueOf(t.getPassDate()));
 		sql  = sql.replace("#{ceate_user_id}",String.valueOf(t.getCeateUserId()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("flow_task_info",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
