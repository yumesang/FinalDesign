...
//				request.setAttribute(SSOConfig.SSO_COOKIE_MAXAGE, 60);
	...
	public ModelAndView hplusindex(HttpServletRequest request) {
		...
		//待办事宜
		TSRoleUser role = systemService.findUniqueByProperty(TSRoleUser.class, "TSUser.id", ResourceUtil.getSessionUser().getId());
		String flowTaskInfoSql = "select t.id, t.task_name, t.url, t.create_date from flow_task_info t where check_user_id like '%"+ role.getTSRole().getId() +"%'  order by create_date desc";			
		List<Object[]> flowTaksInfoList = systemService.findListbySql(flowTaskInfoSql);
		if(flowTaksInfoList.size() == 0){
			flowTaskInfoSql = "select t.id, t.task_name, t.url, t.create_date from flow_task_info t where check_user_id = '"+ ResourceUtil.getSessionUser().getId() +"'  order by create_date desc";	
			flowTaksInfoList = systemService.findListbySql(flowTaskInfoSql);
		}
		
		String showTaskInfo = "<ol><li>";
		for(int i = 0; i < flowTaksInfoList.size(); i++){
			Object[] obj = flowTaksInfoList.get(i);
			String id = obj[0].toString();
			if(i == flowTaksInfoList.size()-1){
				showTaskInfo += "<a onclick=\"showTaskInfoDetail( '"+ id +"','"+ obj[2] +"')\" href=\"#\">" + obj[1] + " ("  + obj[3].toString().substring(0,10) + ")</a></li></ol>";
			}else{
				showTaskInfo += "<a onclick=\"showTaskInfoDetail( '"+ id +"','"+ obj[2] +"')\" href=\"#\">" + obj[1] + " ("  + obj[3].toString().substring(0,10) + ")</a></li><li><a>";
			}
		}
		if(showTaskInfo.equals("<ol><li>")){
			showTaskInfo = null;
		}
		request.setAttribute("showTaskInfo", showTaskInfo);
	...
	@RequestMapping(params = "hplushome2")
